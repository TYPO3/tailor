...
class EmConfVersionReplacer
{
    ...
    public function setVersion(string $newVersion): void
    {
        ...
        if ($fileContents === false) {
            throw new \InvalidArgumentException('The file ' . $this->filePath . ' could not be opened');
        }
        $updatedFileContents = preg_replace_callback('/' . $this->pattern . '/u', function ($matches) use ($newVersion) {
